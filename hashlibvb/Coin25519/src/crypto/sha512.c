...
static __inline void get64(uint64_t *ptr, const unsigned char *data)
{
  int i;
  *ptr = CONST64(0);
  for (i=0; i<8; i++)
  {
    *ptr |= (uint64_t)(data[i]) << (56-8*i);
  }
}

static __inline void put64(uint64_t val, unsigned char *data)
{
  int i;
  for (i=0; i<8; i++)
  {
...
#define GET64(val,data) get64(&(val),data)
...
/* compress 1024-bits */
void static sha512_transform(uint64_t *state, unsigned char *buf)
{
...
int sha512_done(sha512_state * md, unsigned char *out)
{
  ...
  /* store length */
  PUT64(md->length, md->buf + 120);
  sha512_transform(md->state, md->buf);

  /* copy output */
  for (i = 0; i < 8; i += 2) {
    PUT64(md->state[i], out + (8 * i));
    PUT64(md->state[i + 1], out + (8 * (i + 1)));
  }

  return TRUE;
}

int HASHLIBVB_CC sha512(const unsigned char *in, unsigned long inlen, unsigned char *out)
{
  sha512_state state;
  sha512_init(&state);
