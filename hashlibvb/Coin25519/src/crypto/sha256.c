...
static __inline void get32(uint32_t *ptr, const unsigned char *data)
{
  int i;
  *ptr = 0;
...
static __inline void put32(uint32_t val, unsigned char *data)
{
  int i;
  for (i=0; i<4; i++)
  {
...
int HASHLIBVB_CC sha256(const unsigned char *in, unsigned long inlen, unsigned char *out)
{
  sha256_state state;
  sha256_init(&state);
